    ...
    input.addEventListener('click', (e) => {
        ...
        (function InputConfig(clickedValue) {
            if (numScreen.innerText == '0') {
                ...
                if (input.className != 'operators' && clickedValue != '=' && clickedValue != 'C')
                    numScreen.innerText += input.value
        ...
        (function authenticateOperators(clickedValue) {
            ...
            if (input.className == 'operators') {
                ...
                if (numScreen.innerText == '0' && firstNum.length == 0) {
                    ...
                    if (operator.length == 3) { //to control the operator array when we overclick or click more than once
        ...
        (function customiZero(){
            if (input.value == '0' && firstNum.length != 0) {
                secondNum = []
                if (numScreen.innerText == '0') {
                    numScreen.innerText = input.value
                
                }
                secondNum.push(parseFloat(input.value))
...
(function resolveOnOperators(){
    ...
    if(input.className === 'operators' && firstNum.length !=0 && numScreen.innerText != '0'){
        secondNum = []
        if (input.value == '0') {
            numScreen.innerText = input.value
            ...
            let resolved = `${firstNum[0]}${operator[operator.length - 2]}${secondNum[0]}`
            let resolve  =  eval(resolved)
             answerBox.innerText = resolve + ' ' + input.value
             firstNum = []
...
//UTILITY FUNCTION
        function customizedResolve(index) {
            let res = `${parseFloat (firstNum[0])}${operator[operator.length - index]}${parseFloat(secondNum[0])}`
